...
@injectable()
export class SetAccountVerifiedUsecase implements Usecase {
  public constructor(@inject(AccountRepository) private accountRepository: AccountRepository) {}

  public async perform(accountId: string, isVerified: boolean): Promise<AccountEntity> {
    await AccountUsecaseValidations.validateAccountExists(this.accountRepository, accountId);
    return await this.accountRepository.update({ filter: { id: accountId } }, { isVerified });
  }
}
