...
import { ListCredentialsUsecase } from './list-credentials.usecase';

describe(ListCredentialsUsecase, () => {
  let usecase: ListCredentialsUsecase;
  let repo: CredentialMockRepository;

  beforeEach(() => {
    repo = new CredentialMockRepository();
    usecase = new ListCredentialsUsecase(repo);
  });
  ...
  it('returns list of credentials using repo', async () => {
    ...
    expect(repo.list).toBeCalledWith({ limit: 50, offset: 0 });
  });
});
