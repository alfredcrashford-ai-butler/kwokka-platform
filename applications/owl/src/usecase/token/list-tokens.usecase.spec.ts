...
import { ListTokensUsecase } from './list-tokens.usecase';

describe(ListTokensUsecase, () => {
  let usecase: ListTokensUsecase;
  let repo: TokenMockRepository;

  beforeEach(() => {
    repo = new TokenMockRepository();
    usecase = new ListTokensUsecase(repo);
  });
  ...
  it('returns tokens list from repo', async () => {
    ...
    expect(repo.list).toBeCalledWith({ offset: 0, limit: 50 });
  });
});
